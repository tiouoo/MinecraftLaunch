...
public sealed class JavaInstaller
{
    ...
    private async Task ExtractJavaFromManifestAsync(FileInfo manifestFile, CancellationToken token)
    {
            ...
            tasks.Add(Task.Run(async () =>
            {
                ...
                try
                {
                    ...
                    if (!success)
                    {
                        ...
                        Console.WriteLine($"[Java] 下载失败: {fileKey} 原因: {reason} - JavaInstaller.cs:234");
                    }
                }
                finally
                {
        ...
        {
            Console.WriteLine("[Java] 以下文件最终失败： - JavaInstaller.cs:267");
            foreach (var kv in failedFiles)
                Console.WriteLine($"{kv.Key} : {kv.Value} - JavaInstaller.cs:269");
        }
